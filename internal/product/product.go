...
type EcommerceDatabase interface {
	...
	GetShopId(ctx context.Context, shopID string) ([]Shop, error)
...
func (pdb *PostgresDatabase) GetProduct(ctx context.Context, id string) (ProductItem, error) {
	...
	product.Shops, err = pdb.GetShopId(ctx, product.ShopID)
	if err != nil {
		return ProductItem{}, fmt.Errorf("failed to get product images: %v", err)
	}
...
func (pdb *PostgresDatabase) GetShopId(ctx context.Context, shopID string) ([]Shop, error) {
...
func (pdb *PostgresDatabase) GetShops(ctx context.Context) ([]Shop, error) {
	rows, err := pdb.db.QueryContext(ctx, `
				SELECT shop_id, name, description, province, 
				phone, email, image_url FROM shops`)
...
func (s *Store) GetShopId(ctx context.Context, shopID string) ([]Shop, error) {
	return s.db.GetShopId(ctx, shopID)
}

func (s *Store) GetShops(ctx context.Context) ([]Shop, error) {
	return s.db.GetShops(ctx)
}
